...
class Agent_mec:
    def __init__(self, id, com_re, bw_re):
        ...
        self.timesteps = 0
        self.com_outage = False
        self.bw_outage = False
    ...
    def generate_task_for_observation(self):
        ...
        #设置task_id
        task_id = {'current_timestep': self.timesteps, 'destinating agent': self.id}
        task.set_id(task_id)
        task_list = []
        #将生成的任务放进task_list
        task_list.append(task)
        # 将self.task_list中的任务全部放进self.queued_task列表后面，并清空self.task_list
        self.queued_task.extend(task_list)
    ...
    def get_obs(self,action_dict):
        """在执行本函数时，生成新的任务放入queued_taks列表后面，同时观测queued_task列表头部的任务,需要更新任务队列的队首元素"""
        #生成新任务，放到任务队列末尾
        agent_observation = []
        agent_reward = None
        self.generate_task_for_observation()
        #处理本次动作执行时的任务（即任务队列队首的任务）
        current_process_task = self.queued_task[0]
        #获取本次agent动作为本次任务分配的带宽和计算资源
        agent_allocate_BW = action_dict['BW']
        agent_allocate_CPU = action_dict['CPU']
        #这里可以设置分配资源错误计数，但目前本代码尚未设置
        if self.com_left >= 0 and self.com_left-agent_allocate_CPU < 0:
            agent_reward = -10
            pass
        elif self.bw_left >= 0 and self.bw_left - agent_allocate_BW < 0:
            agent_reward = -10
            pass
        #资源分配动作符合实际条件，执行动作
        else:
            self.allocate_resource(current_process_task,agent_allocate_BW,agent_allocate_CPU)
            self.queued_task.remove(current_process_task)
            self.processing_task_list.append(current_process_task)
        #判断是否能在时延约束内完成任务，可以则reward为1
            if current_process_task.trans_time_slot + current_process_task.com_time_slot <= math.ceil(current_process_task.max_delay/GlobalVar.slot)+1:
                self.com_left -= agent_allocate_CPU
                self.bw_left -= agent_allocate_BW
                agent_reward = 1
            else:
                agent_reward = -1
        agent_observation =[self.bw_left, self.com_left,self.queued_task[0].datasize,self.queued_task[0].com_size]
        return agent_observation,agent_reward
...
class Task:
    def __init__(self, com_size, datasize, max_delay):
        ...
        self.ID = None
        print("task with computing requirement: %d and data requirement:%d"%(self.com_size,self.datasize))
    def set_id(self,ID):
        self.ID = ID
...
class Env_mec:
    ...
    def step(self, action):
        ...
        for i,agent in enumerate(self.agents):
            ...
            #各agent的action已就绪
            observation,reward = agent.get_obs(actions)
            print("observation:%s, reward:%s"%(observation,reward))
            print("observation %s of agent %s and current timestep is %d:"%(agent.get_obs(actions),agent.__repr__(),self.current_timestep()))
            obslist.extend(observation) #这里可以看出调用了agent的get_obs方法，得检查其是否得当
